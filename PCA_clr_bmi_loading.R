...
p <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = bmi_group)) +
  ...
  labs(title = "PCA (CLR) - BMI grouping at 30",
       x = xlab,
       y = ylab,
       color = "BMI Group") +
  theme_minimal()
...
# 8) Save
ggsave("PCA_CLR_bmi30.pdf", p, width = 6, height = 5)
cat("PCA with BMI grouping (30) complete. Output: PCA_CLR_bmi30.pdf\n")
